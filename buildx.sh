...
if ! docker buildx inspect skaffold-builder-argocd-image-updater >/dev/null 2>&1; then
  docker buildx create --name skaffold-builder-argocd-image-updater --platform $platforms
fi
...
docker buildx build --builder skaffold-builder-argocd-image-updater --tag $IMAGE $args "$BUILD_CONTEXT"
